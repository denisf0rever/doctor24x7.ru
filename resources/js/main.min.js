...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (fullForm) {
    ...
    fullFormConfirmBtn.onclick = async (event) => {
      event.preventDefault();
      if (fullFormConfirmBtn.classList.contains('full-form__confirm-btn-active')) {
        // Собираем данные формы
        const form = document.querySelector('.comments__form');

        // Преобразуем поле to_answer_id в число, если оно существует
        const formData = new FormData(form);
        if (formData.has('to_answer_id')) {
          const toAnswerId = formData.get('to_answer_id');
          formData.set('to_answer_id', Number(toAnswerId));
        }

        try {
          // Создаём объект для передачи в запрос
          const dataObject = {};
          formData.forEach((value, key) => {
            dataObject[key] = value;
          });

          // Создаём объект для отправки в форме
          const formElement = document.createElement('form');
          formElement.method = form.method;
          formElement.action = form.action;

          // Добавляем данные в форму в виде input элементов
          ...
          formElement.submit(); // Стандартная отправка формы
          document.body.removeChild(formElement); // Удаляем временную форму после отправки
