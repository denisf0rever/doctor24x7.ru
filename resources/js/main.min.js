...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const mobileHeaderMenu = document.querySelector(
        ".header-mobile-menu__wrapper"
    );
    const mobileHeaderMenuInner = document.querySelector(
        ".header-mobile-menu__inner"
    );
    const burgerCLose = document.querySelector(".header-mobile-menu__close");

    // Открытие меню при клике на бургер

    if (burger) {
        burger.onclick = () => {
            mobileHeaderMenu.classList.add(
                "header-mobile-menu__wrapper--opened"
            );
        ...
        document.addEventListener("click", (event) => {
            ...
            if (
                !mobileHeaderMenuInner.contains(event.target) &&
                !burger.contains(event.target)
            ) {
                mobileHeaderMenu.classList.remove(
                    "header-mobile-menu__wrapper--opened"
                );
    ...
    document.querySelectorAll(".Accordion li").forEach((question) => {
        question.addEventListener("click", function () {
            const answerWrapper = this.querySelector(".panel");
    ...
    (() => {
        ...
        const closeChatBtn = document.querySelector(".close-chat-link");
        ...
        if (chatId && userId && messagesList) {
            ...
            const insertCallDoctorButton = () => {
                // if (messagesList.querySelector(".call-doctor-button")) return;
                // const li = document.createElement("li");
                // li.className =
                //     "chat-messages__chat-message chat-message call-doctor-button";
                // li.style.textAlign = "center";
                // li.style.marginTop = "12px";
                // const button = document.createElement("button");
                // button.textContent = "Позвать врача";
                ...
                //     fetch(`${appUrl}chat/messages/${chatId}/request-doctor`, {
                //         method: "POST",
                //         headers: {
                //             "Content-Type": "application/json",
                //         },
                ...
                //         })
                //         .then(() => {
                //             li.remove(); // удалить кнопку после успешного запроса
                //         })
                //         .catch((err) => {
                //             console.error("Ошибка при вызове врача:", err);
            ...
            if (closeChatBtn) {
                closeChatBtn.addEventListener("click", (e) => {
                    ...
                    fetch(`${appUrl}chat/messages/${chatId}/close`, {
                        method: "POST",
                        headers: {
                            "Content-Type": "application/json",
                        },
                        body: JSON.stringify({
                            test: true,
                            message: "Это тестовый POST-запрос",
                        }),
                    })
                        .then((response) => {
                            if (!response.ok) {
    ...
    document
        .querySelectorAll(".expert-card__player-button")
        .forEach((player) => {
            player._isPlaying = false;
            player._audio = null;

            player.addEventListener("click", () => {
                // Остановить другие аудио
                document
                    .querySelectorAll(".expert-card__player-button")
                    .forEach((p) => {
                        if (p !== player) {
                            ...
                            p.src = "/storage/common/play-button.svg";
                            p._isPlaying = false;
                        }
                    });
                ...
                if (!player._audio) {
                    ...
                    player._audio.addEventListener("ended", () => {
                        player.src = "/storage/common/play-button.svg";
                        player._isPlaying = false;
                    });
                }
                ...
                if (player._isPlaying) {
                    ...
                    player._audio.play();
                    player.src = "/storage/common/pause-button.svg";
                }
    ...
    /*ОЦЕНКА СТАТЬИ ЗВЕЗДЫ */

    (() => {
        const articleStars = document.querySelectorAll(
            ".content-block-rating__star"
        );
        let selectedStarsAmount = 0;

        const setActiveStars = (amount) => {
            articleStars.forEach((star, key) => {
                star.src =
                    key <= amount - 1
                        ? "/storage/common/star-on.png"
                        : "/storage/common/star-off.png";
            });
        };

        const selectStars = (selectedIndex) => {
            selectedStarsAmount = selectedIndex;
            setActiveStars(selectedStarsAmount);
        };

        if (articleStars.length > 0) {
            articleStars.forEach((star, index) => {
                const amount = index + 1;

                // Десктоп: подсветка при наведении
                star.addEventListener("mouseover", () =>
                    setActiveStars(amount)
                );
                star.addEventListener("mouseleave", () =>
                    setActiveStars(selectedStarsAmount)
                );

                // Универсальный выбор (десктоп и мобила)
                star.addEventListener("click", () => selectStars(amount));

                // Мобильные: эмуляция hover-подсветки по первому касанию
    ...
    /*МАСКА ИНПУТА ПОПОЛНЕНИЕ БАЛАНСА */

    const balanceInput = document.querySelector(".balance__amount-input");
    const sumHiddenInput = document.querySelector("#balance__amount-hidden");
    ...
    if (balanceInput && sumHiddenInput) {
        balanceInput.addEventListener("input", () => {
            const rawValue = balanceInput.value;
            const numeric = rawValue.replace(/\D/g, "");

            // Обновляем скрытое поле только числовым значением
            sumHiddenInput.value = numeric;

            const formattedNumber = formatCurrency(numeric);
            const finalValue = formattedNumber ? `${formattedNumber} ₽` : "";

            // Сохраняем позицию курсора до символа ₽
            const prevLength = balanceInput.value.length;
            balanceInput.value = finalValue;
            const newLength = finalValue.length;

            const cursorPos = newLength - 2; // перед ₽
            balanceInput.setSelectionRange(cursorPos, cursorPos);
        });
    }
