...
window.onload = () => {
  ...
  const swiperReveal = new Swiper('.stories-reveal__wrapper', {
    ...
    on: {
      click() {
        swiperReveal.slideTo(this.clickedIndex);
        stopPagination(this.clickedIndex);
      },
    },
  });


  storyWrapper.forEach((el, key) => {
    el.onclick = () => {
      //revealing swiper
      storiesReveal.classList.remove('hide');
      body.classList.add('hidden');
      swiperReveal.slideTo(key);
      stopPagination(key);
      //setting slide size
      const storyRevealedWrapper = document.querySelectorAll(".story-revealed__wrapper");
      const h = storyRevealedWrapper[0].offsetHeight;
      storyRevealedWrapper.forEach((el) => {
        if (window.screen.width > 526) {
          el.style.width = `${h / 1.77}px`;
        }
        else {
  ...
  if (close) {
    close.onclick = () => {
      // hiding swiper
      storiesReveal.classList.add('hide');
  ...
  const nestedSwiper = new Swiper(".story-revealed__nested-swiper", {
    allowTouchMove: false,
    autoplay: {
      delay: 10000,
    },
    speed: 1,
    pagination: {
      el: '.story-revealed__pagination',
      bulletClass: 'story-revealed__bullet',
      bulletActiveClass: 'story-revealed__bullet-active',
      renderBullet: function (index, className) {
        return '<span class="' + className + '">' + '<span class="story-revealed__bullet-bg"></span>' + '<span class="story-revealed__bullet-bg-active"></span>' + '</span>';
      },
    },
    on: {
      //color of passed bullets
      slideChange: function () {
        var bullets = document.querySelectorAll('.story-revealed__bullet-active');
        for (let i = 0; i < bullets.length; i++) {
          const prevSib = getPreviousSiblings(bullets[i]);
          const nextSib = getNextSiblings(bullets[i]);
          prevSib.forEach((el, key) => {
            el.classList.add('story-revealed__bullet-passed');
          })
          nextSib.forEach((el, key) => {
            el.classList.remove('story-revealed__bullet-passed');
          })
        }
      },
    },
  ...
  const buttonNext = document.querySelectorAll('.story-revealed__button-next');
  const buttonPrev = document.querySelectorAll('.story-revealed__button-prev');
  const gradientLeft = document.querySelectorAll('.story-revealed__left-gradient');
  const gradientRight = document.querySelectorAll('.story-revealed__right-gradient');
  const storyRevealed = document.querySelectorAll('.story-revealed');
  const storyRevealedList = document.querySelectorAll('.story-revealed__list');
  ...
  const stopPagination = (key) => {
    //stopping autoplay except active 
    for (let i = 0; i < storyRevealed.length; i++) {
      if (i === key) {
        nestedSwiper[i].autoplay.start();
      }
      else {
        nestedSwiper[i].autoplay.stop();
      }
    }
  }

  //buttons slide
  buttonNext.forEach((el, key) => {
    el.onclick = () => {
      if (storyRevealedList[key].children[storyRevealedList[key].children.length - 1].classList.contains("swiper-slide-active")) {
        swiperReveal.slideTo(key + 1);
      }
      else {
        nestedSwiper[key].slideTo(nestedSwiper[key].activeIndex + 1);
      }
    }
  })

  gradientRight.forEach((el, key) => {
    el.onclick = () => {
      if (storyRevealedList[key].children[storyRevealedList[key].children.length - 1].classList.contains("swiper-slide-active")) {
        swiperReveal.slideTo(key + 1);
      }
  ...
  buttonPrev.forEach((el, key) => {
    el.onclick = () => {
      if (storyRevealedList[key].children[0].classList.contains("swiper-slide-active")) {
        swiperReveal.slideTo(key - 1);
      }
      else {
        nestedSwiper[key].slideTo(nestedSwiper[key].activeIndex - 1);
  ...
  //EXPAND COMMETS
  let summary = document.querySelectorAll('.comment__text')
  let summaryFull = [];
  for (let i = 0; i < summary.length; i++) {
    if (summary[i].textContent.length > 570) {
      summaryFull[i] = summary[i].textContent;
      let truncated = summary[i].textContent.substring(0, 570)

      summary[i].innerHTML = `<span>${truncated}... <span class='comment__text-more'>еще</span></span>`
    }
  }
  const summaryMore = document.querySelectorAll('.comment__text-more');
  for (let i = 0; i < summaryMore.length; i++) {
    summaryMore[i].onclick = () => {
      summary[i].innerHTML = `<span>${summaryFull}</span>`
    }
  }
