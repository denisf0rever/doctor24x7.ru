...
document.addEventListener('DOMContentLoaded', function () {
  ...
  /*ОТКРЫТИЕ ЧАТА МОБИЛЬНАЯ ВЕРСИЯ */

  const sidebarItem = document.querySelectorAll('.chat-sidebar__item');
  const chatMainBlock = document.querySelector('.full-chat__main');
  ...
  if (sidebarItem.length > 0 && chatMainBlock && chatBackBtn) {
    sidebarItem.forEach(el => {
      el.addEventListener('click', () => {
        chatMainBlock.classList.add('full-chat__main--show');
      })
    })

    chatBackBtn.addEventListener('click', () => {
      chatMainBlock.classList.remove('full-chat__main--show');
    })
  }
