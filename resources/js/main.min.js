...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const chatMessagesWrapper = document.querySelector(".chat-messages__wrapper");
    ...
    const calculateReceiptPrice = () => {
        let comission = 0, additionalValue = 0;
    ...
    if (
      ...
      if (receptionCheckbox && receiptComission) {
          receptionCheckbox.addEventListener("change", function (e) {
            if (!receiptCustonPriceBtn.classList.contains(
  ...
  if (chatId && userId && messagesList) {
    const messagesURL = `https://doctor24x7.ru/chat/messages/${chatId}`;
    let intervalId = null;
    let lastMessageId = null; // будем хранить ID последнего сообщения, чтобы не дублировать

    const ACTIVE_INTERVAL = 30000;    // 30 секунд
    ...
    const fetchMessages = () => {
      fetch(messagesURL)
        ...
        .then((data) => {
          ...
          if (data.status !== 0) {
            console.log('Статус не равен 0, останавливаем опрос.');
          ...
          const newItems = newMessages.filter(msg => !lastMessageId || msg.id > lastMessageId);

          if (newItems.length > 0) {
            renderMessages(newItems);
    ...
    const renderMessages = (messages) => {
      messages.forEach(msg => {
        ...
        wrapper.className = 'chat-message__wrapper' + (isOwn ? ' chat-message--own' : '');
