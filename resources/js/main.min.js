...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const chatAdditionalFields = document.querySelector('.chat__additional-fields');
  const chatFooterSecond = document.querySelector('.chat__footer-second');

  if (chatNextButton && chatBody && chatAdditionalFields  && chatFooterSecond) {
    chatNextButton.onclick = () => {
      if (chatNextButton.classList.contains('chat__button-continue--active')) {
  ...
  if (chatPrevButton && chatBody && chatAdditionalFields && chatFooterSecond) {
  ...
  if (chatTextarea) {
    chatTextarea.addEventListener('input', () => {
      ...
      if (chatTextarea.value.length > 10) {
        ...
        chatNextButton.classList.remove('chat__button-continue--active');
      }
    })
  }
  ...
  const expertsSlider = new Swiper(".experts-list__wrapper", {
    navigation: {
...
document.querySelectorAll('.question-accordion').forEach(question => {
  question.addEventListener('click', function () {
    const answerWrapper = this.querySelector('.question-accordion__ansver-wrapper');
    if (this.classList.contains('question-accordion__opened')) {
      // Animate collapse
      answerWrapper.style.maxHeight = `${answerWrapper.scrollHeight + 20}px`;
      answerWrapper.style.paddingTop = `0`;
      setTimeout(() => {
        answerWrapper.style.maxHeight = '0';
        this.classList.remove('question-accordion__opened');
      }, 0);
    } else {
      // Animate expansion
      this.classList.add('question-accordion__opened');
      answerWrapper.style.maxHeight = `${answerWrapper.scrollHeight + 20}px`;
