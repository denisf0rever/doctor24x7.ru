        ...
        if (chatId && userId && messagesList) {
            ...
            if (textarea && form) {
                form.addEventListener("submit", function (event) {
                    if (isSubmited) {
                        event.preventDefault(); // Блокируем повторную отправку
                        return;
                    }
                ...
                textarea.addEventListener("keydown", function (event) {
                    if (isSubmited) {
                        event.preventDefault(); // блокируем всё, включая перенос строки
                        return;
                    }

                    if (event.key === "Enter" && !event.shiftKey) {
                        event.preventDefault();
                        form.requestSubmit(); // вызывает submit + триггерит обработчик выше
                    }
                });
            }
