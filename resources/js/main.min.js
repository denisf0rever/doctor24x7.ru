    ...
    fullFormConfirmBtn.onclick = async (event) => {
      ...
      if (fullFormConfirmBtn.classList.contains('full-form__confirm-btn-active')) {
        ...
        if (formData.has('to_answer_id')) {
          formData.set('to_answer_id', Number(formData.get('to_answer_id')));
        }

        // Проверяем, что поле преобразовано в число
        // console.log(formData.get('to_answer_id')); // Это должно быть числом
