...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const formatCurrency = (value) => {
        const numeric = value.replace(/\D/g, ""); // Удаляем всё, кроме цифр
        if (!numeric) return "";

        // Форматируем с пробелами и добавляем знак ₽
    ...
    if (balanceInput) {
        balanceInput.addEventListener("input", (e) => {
            ...
            balanceInput.value = formatted;

            // Обновим позицию курсора (простая логика, не идеально сохраняет)
            const diff = balanceInput.value.length - rawValue.length;
            balanceInput.setSelectionRange(
                cursorPosition + diff,
                cursorPosition + diff
            );
        });
