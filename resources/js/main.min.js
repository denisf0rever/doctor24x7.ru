  ...
  if (likeComments.length > 0) {
    likeComments.forEach(comment => {
      ...
      likelink.addEventListener('click', function (event) {
        ...
        if (!likeImg.classList.contains('comment__like-img-active')) {
          likesAmount.innerHTML = +likesAmount.innerHTML + 1;
          likeImg.classList.add('comment__like-img-active');
          const url = this.href; // получаем адрес ссылки
          if (csrfToken) {
            fetch(url, {
              method: 'POST',
              headers: {
          ...
          likesAmount.innerHTML = +likesAmount.innerHTML - 1;
          likeImg.classList.remove('comment__like-img-active');
          const url = this.href; // получаем адрес ссылки
          if (csrfToken) {
            fetch(url, {
              method: 'POST',
      ...
      dislikelink.addEventListener('click', function (event) {
        event.preventDefault(); // предотвращаем переход по ссылке
        if (!dislikeImg.classList.contains('comment__dislike-img-active')) {
          dislikeImg.classList.add('comment__dislike-img-active');
          const url = this.href; // получаем адрес ссылки
          if (csrfToken) {
            fetch(url, {
              method: 'POST',
          ...
          dislikeImg.classList.remove('comment__dislike-img-active');
          const url = this.href; // получаем адрес ссылки
          if (csrfToken) {
            fetch(url, {
              method: 'POST',
