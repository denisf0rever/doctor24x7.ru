...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const selectconsultationTub = (direction) => {
    if (direction === 'next') {
      consultationTubs.forEach((el, key) => {
        console.log('aoaoaoa---', el.getAttribute('data-step'), activeTub + 1)
  ...
  const checkValidationTitle = (validationTitle) => {
    if (validationTitle.getAttribute('name') === 'title') {
      const inputValue = validationTitle.value.trim(); // Убираем лишние пробелы
      const errorText = "Текст должен быть длиннее 10 символов";
      ...
      if (inputValue.length <= 10) {
        ...
        validationTitle.insertAdjacentElement('afterend', errorDiv);
        validationTitle.classList.add('consultation-form__input-error');
      } else {
        console.log('Валидация пройдена:', inputValue);
        validationTitle.classList.remove('consultation-form__input-error');
      }
    }
  }
  ...
  if (validationItem.length > 0) {
    validationItem.forEach(el => {
      el.querySelector('input').addEventListener('blur', function () {
        console.log('Фокус снят с инпута');
        if (this.getAttribute('name') === 'title') {
          checkValidationTitle(this);
        }
      });
    });
  }
  ...
  if (consultationOptions.length > 0) {
    consultationOptions.forEach(el => {
      el.onclick = () => {
        console.log(rubricInput.value, '------', el.value);
